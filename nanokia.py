...
class Nanokia:
    ...
    def list_devices(self, xml_file):
        ...
        try:
            ...
            # Parse Device entries (simpler structure, more common)
            device_pattern = r'<Device\.\d+\.[^>]*>.*?</Device\.\d+\.>'
            devices = re.findall(device_pattern, content, re.DOTALL)
            
            # Also parse Host entries (detailed structure)
            host_pattern = r'<Host\.\d+\..*?>.*?</Host\.\d+\.>'
            hosts = re.findall(host_pattern, content, re.DOTALL)
            
            all_devices = devices + hosts
            ...
            print("\n" + "="*150)
            print(f"{Colors.GREEN}{'CONNECTED DEVICES - DETAILED VIEW':^150}{Colors.RESET}")
            print("="*150)
            print(f"{Colors.CYAN}{'#':<3} {'Hostname':<25} {'IP Address':<16} {'MAC Address':<18} {'Type':<10} {'Source':<8} {'Status':<8} {'Sessions':<10} {'Last Active':<20}{Colors.RESET}")
            print("-"*150)
            
            device_count = 0
            for i, device in enumerate(all_devices, 1):
                # Extract hostname (try both patterns)
                hostname_match = re.search(r'<HostName[^>]*v="([^"]+)"', device)
                if not hostname_match:
                    hostname_match = re.search(r'<X_ASB_COM_HostName[^>]*v="([^"]+)"', device)
                ...
                ip_match = re.search(r'<IPAddress[^>]*v="([^"]+)"', device)
                if not ip_match:
                    ip_match = re.search(r'<X_ASB_COM_IPAddress[^>]*v="([^"]+)"', device)
                ip = ip_match.group(1) if ip_match and ip_match.group(1) != "" else "N/A"
                
                # Skip invalid entries
                if mac == "N/A":
                    continue
                if ip == "Invalid" or ip == "" or ip == "0.0.0.0":
                    ip = "N/A"
                
                # Generate hostname from MAC if not found
                if not hostname:
                    hostname = f"Unknown_{mac[-8:].replace(':', '')}"
                if len(hostname) > 24:
                    hostname = hostname[:21] + "..."
                
                # Extract interface type (WiFi or Ethernet)
                interface_match = re.search(r'<InterfaceType[^>]*v="([^"]+)"', device)
                interface = interface_match.group(1) if interface_match else "N/A"
                if interface == "802.11":
                    interface = "WiFi"
                ...
                # Extract address source (DHCP/Static)
                source_match = re.search(r'<AddressSource[^>]*v="([^"]+)"', device)
                source = source_match.group(1) if source_match else "N/A"
                if source != "N/A":
                    source = source[:7]
                
                # Extract active status
                status_match = re.search(r'<Active[^>]*v="(\d+)"', device)
                if not status_match:
                    status_match = re.search(r'<X_ASB_COM_Status[^>]*v="(\d+)"', device)
                status_code = status_match.group(1) if status_match else "0"
                
                # Skip devices with status 0 (disconnected/never connected)
                if status_code == "0":
                    continue
                
                status = "Active" if status_code == "1" else "Inactive"
                
                # Extract NAT session count
                nat_match = re.search(r'<X_ALU_COM_NumberOfCurrentNATSession[^>]*v="(\d+)"', device)
                nat_sessions = nat_match.group(1) if nat_match else "0"
                
                # Extract lease time remaining (for DHCP)
                lease_match = re.search(r'<LeaseTimeRemaining[^>]*v="(\d+)"', device)
                lease_time = int(lease_match.group(1)) if lease_match else 0
                
                # Extract IPv6 (from nested X_ASB_COM_DeviceIPv6Address or lan_statusV6)
                ipv6 = "N/A"
                ipv6_match = re.search(r'<IPv6Address[^>]*v="([^"]+)"', device)
                if ipv6_match and ipv6_match.group(1) and ipv6_match.group(1) != "":
                    ipv6 = ipv6_match.group(1)
                    if len(ipv6) > 25:
                        ipv6 = ipv6[:22] + "..."
                
                # Extract last active time
                last_active_match = re.search(r'<X_ALU_COM_LastActiveTime[^>]*v="([^"]+)"', device)
                last_active = last_active_match.group(1) if last_active_match else "N/A"
                if last_active != "N/A" and last_active != "":
                    # Clean up the timestamp
                    last_active = last_active.replace(" AM", "").replace(" PM", "")
                    if len(last_active) > 19:
                ...
                # Color coding for interface type
                if interface == "WiFi":
                    interface_colored = f"{Colors.CYAN}{interface}{Colors.RESET}"
                elif interface == "Ethernet":
                    interface_colored = f"{Colors.BLUE}{interface}{Colors.RESET}"
                else:
                    interface_colored = interface
                
                # Format NAT sessions with color
                nat_sessions_int = int(nat_sessions) if nat_sessions.isdigit() else 0
                if nat_sessions_int > 0:
                    nat_colored = f"{Colors.GREEN}{nat_sessions}{Colors.RESET}"
                else:
                    nat_colored = nat_sessions
                
                print(f"{device_count:<3} {hostname:<25} {ip:<16} {mac:<18} {interface_colored:<18} {source:<8} {status_colored:<16} {nat_colored:<18} {last_active:<20}")
            
            print("="*150)
            self.log(f"Found {device_count} connected device(s)", Colors.GREEN, "[+]")
            
            # Show summary statistics
            print(f"\n{Colors.CYAN}[INFO] Device Summary:{Colors.RESET}")
            
            # Count devices by status
            status_1_count = len(re.findall(r'<X_ASB_COM_Status[^>]*v="1"', content))
            status_2_count = len(re.findall(r'<X_ASB_COM_Status[^>]*v="2"', content))
            
            if status_1_count > 0:
                print(f"  → Active devices: {status_1_count}")
            if status_2_count > 0:
                print(f"  → Inactive devices: {status_2_count}")
            
            # Count devices with IP addresses
            valid_ips = len(re.findall(r'<X_ASB_COM_IPAddress[^>]*v="(?!0\\.0\\.0\\.0|Invalid)[0-9.]+">>', content))
            if valid_ips > 0:
                print(f"  → Devices with IP: {valid_ips}")
            
            # Count IPv6 addresses
            ipv6_count = len(re.findall(r'<IPv6Address[^>]*v="(?!\\s*$)[^"]+"', content))
            if ipv6_count > 0:
            ...
            import traceback
            self.verbose_log(traceback.format_exc())
    
    def list_devices_live(self):
        """Fetch and display real-time connected devices from router"""
        self.log("Fetching live device information from router...", Colors.YELLOW)
        
        try:
            # Fetch the LAN status page
            response = self.session.get(f"{self.target}/lan_status.cgi?wlan")
            
            if response.status_code != 200:
                self.log(f"Failed to fetch device list: HTTP {response.status_code}", Colors.RED, "[ERROR]")
                return
            
            # Extract device_cfg JavaScript array
            device_cfg_match = re.search(r'var device_cfg = (\[.*?\]);', response.text, re.DOTALL)
            
            if not device_cfg_match:
                self.log("Could not find device_cfg in response", Colors.RED, "[ERROR]")
                return
            
            # Parse the JavaScript array (it's almost JSON, needs cleanup)
            device_cfg_str = device_cfg_match.group(1)
            
            # Extract individual device objects using regex
            device_pattern = r'\{[^}]*HostName[^}]*?\}'
            devices = re.findall(device_pattern, device_cfg_str, re.DOTALL)
            
            if not devices:
                self.log("No devices found", Colors.YELLOW, "[INFO]")
                return
            
            print("\n" + "="*150)
            print(f"{Colors.GREEN}{'LIVE CONNECTED DEVICES':^150}{Colors.RESET}")
            print("="*150)
            print(f"{Colors.CYAN}{'#':<3} {'Hostname':<25} {'IP Address':<16} {'MAC Address':<18} {'Type':<10} {'Source':<8} {'Status':<8} {'Lease Time':<12} {'Sessions':<10}{Colors.RESET}")
            print("-"*150)
            ...
            for device_str in devices:
                # Parse fields from JavaScript object
                hostname_match = re.search(r'["\']?HostName["\']?\s*:\s*["\']([^"\']+)["\']', device_str)
                hostname = hostname_match.group(1) if hostname_match else "Unknown"
                
                # Skip if hostname is "Unknown_" pattern (these are placeholder names)
                if hostname.startswith("Unknown_"):
                    # Try to get a better name from the MAC
                    pass
                ...
                # Extract IP
                ip_match = re.search(r'IPAddress\s*:\s*["\']([^"\']+)["\']', device_str)
                ip = ip_match.group(1) if ip_match else "N/A"
                
                # Extract MAC
                mac_match = re.search(r'MACAddress\s*:\s*["\']([^"\']+)["\']', device_str)
                mac = mac_match.group(1) if mac_match else "N/A"
                ...
                interface_match = re.search(r'InterfaceType\s*:\s*["\']([^"\']+)["\']', device_str)
                interface = interface_match.group(1) if interface_match else "N/A"
                if interface == "802.11":
                    interface = "WiFi"
                elif interface == "Ethernet":
                ...
                source_match = re.search(r'AddressSource\s*:\s*["\']([^"\']+)["\']', device_str)
                source = source_match.group(1) if source_match else "N/A"
                if source != "N/A":
                    source = source[:7]
                
                # Extract Active status
                active_match = re.search(r'Active\s*:\s*(\d+)', device_str)
                ...
                # Extract Lease Time
                lease_match = re.search(r'LeaseTimeRemaining\s*:\s*(\d+)', device_str)
                lease_sec = int(lease_match.group(1)) if lease_match else 0
                if lease_sec > 0:
                    lease_hours = lease_sec // 3600
                    lease_mins = (lease_sec % 3600) // 60
                    lease_time = f"{lease_hours}h {lease_mins}m"
                else:
                    lease_time = "N/A"
                
                # Extract NAT sessions
                nat_match = re.search(r'X_ALU_COM_NumberOfCurrentNATSession["\']?\s*:\s*(\d+)', device_str)
                nat_sessions = nat_match.group(1) if nat_match else "0"
                ...
                if int(nat_sessions) > 0:
                    nat_colored = f"{Colors.GREEN}{nat_sessions}{Colors.RESET}"
                else:
                    nat_colored = nat_sessions
                
                print(f"{device_count:<3} {hostname:<25} {ip:<16} {mac:<18} {interface_colored:<18} {source:<8} {status_colored:<16} {lease_time:<12} {nat_colored:<18}")
            
            print("="*150)
            self.log(f"Found {device_count} device(s) - {active_count} active, {device_count - active_count} inactive", Colors.GREEN, "[+]")
            
            # Show WiFi SSIDs
            ssid_match = re.search(r'var wlan_ssid = (\{.*?\});', response.text, re.DOTALL)
            if ssid_match:
                ssid_data = ssid_match.group(1)
                ssids = re.findall(r'["\']SSID["\']:\s*["\']([^"\']+)["\']', ssid_data)
                if ssids:
                    print(f"\n{Colors.CYAN}[INFO] WiFi Networks:{Colors.RESET}")
                    for i, ssid in enumerate(ssids[:4], 1):
                        print(f"  → {ssid}")
            
            print()
            
        except Exception as e:
            self.log(f"Error fetching live devices: {e}", Colors.RED, "[ERROR]")
            if self.verbose:
                import traceback
                traceback.print_exc()
    ...
    def dump_port_forwarding(self, xml_file):
        """Extract and display port forwarding rules"""
        self.log("Extracting port forwarding rules...", Colors.YELLOW)
        
        try:
            with open(xml_file, 'r', encoding='utf-8') as f:
                content = f.read()
            
            # Find PortMapping entries
            port_pattern = r'<PortMapping\.\d+\..*?</PortMapping\.\d+\.>'
            port_mappings = re.findall(port_pattern, content, re.DOTALL)
            
            if not port_mappings:
                self.log("No port forwarding rules found", Colors.YELLOW, "[INFO]")
                return
            
            print("\n" + "="*120)
            print(f"{Colors.GREEN}{'PORT FORWARDING RULES':^120}{Colors.RESET}")
            print("="*120)
            print(f"{Colors.CYAN}{'#':<3} {'Enabled':<9} {'Name':<30} {'Protocol':<10} {'Ext Port':<12} {'Int IP':<16} {'Int Port':<12}{Colors.RESET}")
            print("-"*120)
            
            rule_count = 0
            for port_map in port_mappings:
                # Extract fields
                enabled = re.search(r'<PortMappingEnabled[^>]*v="([^"]*)"', port_map)
                enabled = enabled.group(1) if enabled else "false"
                
                name = re.search(r'<PortMappingDescription[^>]*v="([^"]*)"', port_map)
                name = name.group(1) if name and name.group(1) else "Unnamed"
                if len(name) > 29:
                    name = name[:26] + "..."
                
                protocol = re.search(r'<PortMappingProtocol[^>]*v="([^"]*)"', port_map)
                protocol = protocol.group(1) if protocol else "N/A"
                
                ext_port = re.search(r'<ExternalPort[^>]*v="([^"]*)"', port_map)
                ext_port_str = ext_port.group(1) if ext_port else "N/A"
                
                # Check for port range
                ext_port_end = re.search(r'<X_ASB_COM_ExternalPortEnd[^>]*v="([^"]*)"', port_map)
                if ext_port_end and ext_port_end.group(1) != ext_port_str:
                    ext_port_str = f"{ext_port_str}-{ext_port_end.group(1)}"
                
                internal_client = re.search(r'<InternalClient[^>]*v="([^"]*)"', port_map)
                internal_client = internal_client.group(1) if internal_client else "N/A"
                
                int_port = re.search(r'<InternalPort[^>]*v="([^"]*)"', port_map)
                int_port_str = int_port.group(1) if int_port else "N/A"
                
                # Check for internal port range
                int_port_end = re.search(r'<X_ASB_COM_InternalPortEnd[^>]*v="([^"]*)"', port_map)
                if int_port_end and int_port_end.group(1) != int_port_str:
                    int_port_str = f"{int_port_str}-{int_port_end.group(1)}"
                
                rule_count += 1
                
                # Color code enabled status
                if enabled.lower() == "true" or enabled == "1":
                    enabled_colored = f"{Colors.GREEN}Enabled{Colors.RESET}"
                else:
                    enabled_colored = f"{Colors.RED}Disabled{Colors.RESET}"
                
                print(f"{rule_count:<3} {enabled_colored:<17} {name:<30} {protocol:<10} {ext_port_str:<12} {internal_client:<16} {int_port_str:<12}")
            
            print("="*120)
            self.log(f"Found {rule_count} port forwarding rule(s)", Colors.GREEN, "[+]")
            print()
            
        except FileNotFoundError:
            self.log(f"Config file not found: {xml_file}", Colors.RED, "[ERROR]")
        except Exception as e:
            self.log(f"Error extracting port forwarding: {e}", Colors.RED, "[ERROR]")
    
    def dump_router_info(self, xml_file):
        """Display router hardware and system information"""
        self.log("Extracting router information...", Colors.YELLOW)
        ...
        try:
            ...
            # Hardware Info
            model = re.search(r'<ModelName[^>]*v="([^"]+)"', content)
            if model:
                print(f"{Colors.CYAN}Model:{Colors.RESET}              {model.group(1)}")
            
            manufacturer = re.search(r'<Manufacturer[^>]*v="([^"]+)"', content)
            if manufacturer:
                print(f"{Colors.CYAN}Manufacturer:{Colors.RESET}       {manufacturer.group(1)}")
            
            serial = re.search(r'<SerialNumber[^>]*v="([^"]+)"', content)
            if serial:
                print(f"{Colors.CYAN}Serial Number:{Colors.RESET}      {serial.group(1)}")
            
            hw_version = re.search(r'<HardwareVersion[^>]*v="([^"]+)"', content)
            if hw_version:
                print(f"{Colors.CYAN}Hardware Version:{Colors.RESET}   {hw_version.group(1)}")
            
            sw_version = re.search(r'<SoftwareVersion[^>]*v="([^"]+)"', content)
            if sw_version:
                print(f"{Colors.CYAN}Software Version:{Colors.RESET}   {sw_version.group(1)}")
            
            # Network Info
            print(f"\n{Colors.CYAN}{'Network Configuration:'}{Colors.RESET}")
            
            wan_ip = re.search(r'<ExternalIPAddress[^>]*v="([0-9.]+)"', content)
            if wan_ip and wan_ip.group(1) != "0.0.0.0":
                print(f"  WAN IP Address:     {wan_ip.group(1)}")
            
            gateway = re.search(r'<DefaultGateway[^>]*v="([0-9.]+)"', content)
            if gateway and gateway.group(1) != "0.0.0.0":
                print(f"  Default Gateway:    {gateway.group(1)}")
            
            dns_servers = re.search(r'<DNSServers[^>]*v="([^"]+)"', content)
            if dns_servers and dns_servers.group(1):
                dns_list = dns_servers.group(1).split(',')
                for i, dns in enumerate(dns_list[:3], 1):
                    print(f"  DNS Server {i}:       {dns}")
            
            # System Stats
            print(f"\n{Colors.CYAN}{'System Status:'}{Colors.RESET}")
            
            uptime_matches = re.findall(r'<UpTime[^>]*v="(\d+)"', content)
            if uptime_matches:
                uptime_sec = max([int(u) for u in uptime_matches if int(u) > 0], default=0)
                if uptime_sec > 0:
                    days = uptime_sec // 86400
                    hours = (uptime_sec % 86400) // 3600
                    minutes = (uptime_sec % 3600) // 60
                    print(f"  Uptime:             {days}d {hours}h {minutes}m ({uptime_sec}s)")
            
            cpu_matches = re.findall(r'<CPUUsage[^>]*v="(\d+)"', content)
            if cpu_matches:
                cpus = [int(c) for c in cpu_matches if int(c) > 0]
                if cpus:
                    cpu = max(cpus)
                    print(f"  CPU Usage:          {cpu}%")
            
            mem_total = re.search(r'<TotalMemory[^>]*v="(\d+)"', content)
            mem_free = re.search(r'<FreeMemory[^>]*v="(\d+)"', content)
            if mem_total and mem_free:
                total_mb = int(mem_total.group(1)) / 1024
                free_mb = int(mem_free.group(1)) / 1024
                used_mb = total_mb - free_mb
                usage_pct = (used_mb / total_mb) * 100 if total_mb > 0 else 0
                print(f"  Memory:             {used_mb:.1f}MB / {total_mb:.1f}MB ({usage_pct:.1f}% used)")
            
            print("="*80)
            self.log("Router information extracted", Colors.GREEN, "[+]")
            print()
            
        except FileNotFoundError:
            self.log(f"Config file not found: {xml_file}", Colors.RED, "[ERROR]")
        except Exception as e:
            self.log(f"Error extracting router info: {e}", Colors.RED, "[ERROR]")
    
    def dump_network_stats(self, xml_file):
        """Display network traffic statistics"""
        self.log("Extracting network statistics...", Colors.YELLOW)
        ...
        try:
            ...
            print("\n" + "="*80)
            print(f"{Colors.GREEN}{'NETWORK TRAFFIC STATISTICS':^80}{Colors.RESET}")
            print("="*80)
            
            # Total bytes
            tx_bytes_matches = re.findall(r'<TotalBytesSent[^>]*v="(\d+)"', content)
            rx_bytes_matches = re.findall(r'<TotalBytesReceived[^>]*v="(\d+)"', content)
            
            if tx_bytes_matches and rx_bytes_matches:
                total_tx = sum([int(b) for b in tx_bytes_matches])
                total_rx = sum([int(b) for b in rx_bytes_matches])
                ...
                print(f"{Colors.CYAN}Total Upload:{Colors.RESET}")
                if tx_gb > 1:
                    print(f"  {tx_gb:.2f} GB ({total_tx:,} bytes)")
                else:
                    print(f"  {tx_mb:.2f} MB ({total_tx:,} bytes)")
                
                print(f"\n{Colors.CYAN}Total Download:{Colors.RESET}")
                if rx_gb > 1:
                    print(f"  {rx_gb:.2f} GB ({total_rx:,} bytes)")
                else:
                    print(f"  {rx_mb:.2f} MB ({total_rx:,} bytes)")
                
                print(f"\n{Colors.CYAN}Combined Total:{Colors.RESET}")
            ...
            if tx_packets_matches and rx_packets_matches:
                total_tx_pkts = sum([int(p) for p in tx_packets_matches])
                total_rx_pkts = sum([int(p) for p in rx_packets_matches])
                
                print(f"\n{Colors.CYAN}Packet Statistics:{Colors.RESET}")
                print(f"  Packets Sent:       {total_tx_pkts:,}")
                print(f"  Packets Received:   {total_rx_pkts:,}")
                print(f"  Total Packets:      {total_tx_pkts + total_rx_pkts:,}")
            
            # Error statistics
            tx_errors = re.findall(r'<TotalErrorsSent[^>]*v="(\d+)"', content)
            rx_errors = re.findall(r'<TotalErrorsReceived[^>]*v="(\d+)"', content)
            
            if tx_errors or rx_errors:
                total_tx_err = sum([int(e) for e in tx_errors]) if tx_errors else 0
                total_rx_err = sum([int(e) for e in rx_errors]) if rx_errors else 0
                
                if total_tx_err > 0 or total_rx_err > 0:
                    print(f"\n{Colors.YELLOW}Error Statistics:{Colors.RESET}")
                    print(f"  Send Errors:        {total_tx_err}")
                    print(f"  Receive Errors:     {total_rx_err}")
            
            print("="*80)
            self.log("Network statistics extracted", Colors.GREEN, "[+]")
            print()
            
        except FileNotFoundError:
            self.log(f"Config file not found: {xml_file}", Colors.RED, "[ERROR]")
        except Exception as e:
            self.log(f"Error extracting network stats: {e}", Colors.RED, "[ERROR]")
...
def show_menu():
    ...
    print(f"{Colors.GREEN}[11]{Colors.RESET} {Colors.BOLD}List Connected Devices (Live){Colors.RESET}")
...
def main():
    ...
    parser.add_argument(
        '--list-devices',
        action='store_true',
        help='List connected devices (from config file)'
    )
    parser.add_argument(
        '--list-devices-live',
        action='store_true',
        help='List connected devices (live from router - shows real hostnames)'
    )
    parser.add_argument(
        '--dump-wifi',
        action='store_true',
    ...
    if direct_mode:
        ...
        # For live device listing, we don't need config file
        if args.list_devices_live:
            exploit.list_devices_live()
            return 0
    ...
    while True:
        ...
        show_menu()
        choice = input(f"{Colors.CYAN}[?] Select option [0-11]: {Colors.RESET}").strip()
        ...
        # Option 11 (Live devices) doesn't need config file
        if choice == '11':
            exploit.list_devices_live()
        else:
            # Download and decrypt config for all other options
            print(f"{Colors.YELLOW}[*] Downloading configuration...{Colors.RESET}")
            if not exploit.download_config():
                exploit.log("Download failed", Colors.RED, "[FAIL]")
                cont = input(f"\n{Colors.CYAN}[?] Try another operation? (y/n): {Colors.RESET}").strip().lower()
